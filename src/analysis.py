...
if __name__ == "__main__":
    ...
    min_date = takeout_data['date'].min()
    max_date = takeout_data['date'].max()
    diff = max_date-min_date

    print(f"First date: {min_date}")
    print(f"Last date: {max_date}")

    print(f"Size: {takeout_data.shape}")

    print(f"Number of days in between first and last dates {diff.days}")

    num_distinct_days = takeout_data['date'].unique()
    print(f"Distinct days: {num_distinct_days.shape[0]}")

    day_with_most_information = takeout_data.groupby('date').count()

    print(takeout_data.loc(takeout_data['date'] == "2018-01-18"))

    print('most information:', day_with_most_information)
